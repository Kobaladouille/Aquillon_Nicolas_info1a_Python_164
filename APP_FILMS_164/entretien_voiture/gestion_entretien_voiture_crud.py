"""
    Fichier : gestion_entretien_voiture_crud.py
    Auteur : OM 2021.05.01
    Gestions des "routes" FLASK et des données pour l'association entre les entretien et les voiture.
"""
...
"""
    ...
    But : Afficher les entretien avec les voiture associés pour chaque film.
    
    Paramètres : id_genre_sel = 0 >> tous les entretien.
...
def films_genres_afficher(id_film_sel):
        ...
        try:
            with DBconnection() as mc_afficher:
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    ...
                    flash(f"Données entretien et voiture affichés !!", "success")
    ...
    return render_template("entretien_voiture/entretien_voiture_afficher.html", data=data_genres_films_afficher)
...
"""
    ...
    Récupère la liste de tous les voiture du film sélectionné par le bouton "MODIFIER" de "entretien_voiture_afficher.html"
    
    Dans une liste déroulante particulière (tags-selector-tagselect), on voit :
    1) Tous les voiture contenus dans la "t_genre".
    2) Les voiture attribués au film selectionné.
    3) Les voiture non-attribués au film sélectionné.
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            ...
            # Récupère la valeur de "id_film" du formulaire html "entretien_voiture_afficher.html"
            # l'utilisateur clique sur le bouton "Modifier" et on récupère la valeur de "id_film"
            # grâce à la variable "id_film_genres_edit_html" dans le fichier "entretien_voiture_afficher.html"
            ...
            # 2) Sélection des voiture "déjà" attribués pour le film.
            # 3) Sélection des voiture "pas encore" attribués pour le film choisi.
            ...
            lst_data_film_selected = [item['id_entretien'] for item in data_genre_film_selected]
            print("lst_data_film_selected  ", lst_data_film_selected,
                  type(lst_data_film_selected))

            # Dans le composant "tags-selector-tagselect" on doit connaître
            # les voiture qui ne sont pas encore sélectionnés.
            lst_data_genres_films_non_attribues = [item['id_voiture'] for item in data_genres_films_non_attribues]
            ...
            # Dans le composant "tags-selector-tagselect" on doit connaître
            # les voiture qui sont déjà sélectionnés.
            lst_data_genres_films_old_attribues = [item['id_voiture'] for item in data_genres_films_attribues]
            ...
            lst_data_genres_films_non_attribues = [item['marque'] for item in data_genres_films_non_attribues]
    ...
    return render_template("entretien_voiture/entretien_voiture_modifier_tags_dropbox.html",
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            # Récupère la liste des voiture qui ne sont pas associés au film sélectionné.
            ...
            # Récupère la liste des voiture qui sont associés au film sélectionné.
            ...
            # Récupère ce que l'utilisateur veut modifier comme voiture dans le composant "tags-selector-tagselect"
            ...
            with DBconnection() as mconn_bd:
                # Pour le film sélectionné, parcourir la liste des voiture à INSÉRER dans la "t_genre_film".
                ...
                # Pour le film sélectionné, parcourir la liste des voiture à EFFACER dans la "t_genre_film".
                # Si la liste est vide, la boucle n'est pas parcourue.
    ...
    # on affiche les entretien et le(urs) genre(s) associé(s).
...
"""
    ...
    Récupère la liste de tous les voiture du film sélectionné par le bouton "MODIFIER" de "entretien_voiture_afficher.html"
    Nécessaire pour afficher tous les "TAGS" des voiture, ainsi l'utilisateur voit les voiture à disposition
